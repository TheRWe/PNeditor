...
\todo{jako poznámka}
Táto kapitola byla inspirovaná a čerpala informace z 
\cite{reisig2013understanding}
...
$$ N = \langle P,T,F,M_{0}\rangle $$
\begin{itemize}
  \item $N$ je Petriho sítí
  \item $P$ je konečná množina míst
  \item $T$ je konečná množina přechodů
  \item $F$ je konečná množina hran
    $ F \subseteq (P \times T) \cup (T \times P) $ \\
    pro $(P \times T)$ používáme také značení $^\circ t$ a pro $(T \times P)$ značení $t^\circ$
  \item $M_{0} : P \rightarrow \mathbb{N}_0$ je počáteční ohodnocení míst 
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{EnabledTransitions}{$P,t,F,M$}
      \State $ $
    \EndFunction
  \end{algorithmic}      
\end{algorithm}      
...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
  \Function{MakeCoverabilityGraph}{$\langle P,T,F,M_0\rangle$}
    \State $\langle V,E,v_0\rangle := \langle\{M_0\},\emptyset,M_0\rangle$
    \State $WorkSet := \emptyset $
    \ForAll{$T \in EnabledTransitions(M_0)$}
    ...
    \While{$WorkSet \neq \emptyset$}
      ...
      \If{$M' \notin V$}
        \State $V := V \cup \{M'\}$
        \ForAll{$T \in EnabledTransitions(M')$}
